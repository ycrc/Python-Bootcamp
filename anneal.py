...
class SimAnneal(object):
    def __init__(self, coords, T=-1, alpha=-1, stopping_T=-1, stopping_iter=-1, id=-1, initial="greedy"):
        self.coords = coords
        self.N = len(coords)
        ...
        if initial=='greedy':
            self.cur_solution = self.initial_solution_greedy()
        elif initial=='shuffle':
            self.cur_solution = self.initial_solution_shuffle()
        else:
            raise Exception("Bad initial method")
    ...
    def initial_solution_shuffle(self):
        """
        Simply shuffle nodes to get an initial solution 
        """
        solution=self.nodes[:]
